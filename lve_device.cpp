...
LveDevice::LveDevice(LveWindow &window) : window{window} {
  createInstance(); // creating vulkan instances, initializes vulkan library between our application with vulkan
  setupDebugMessenger(); // setup validation layer, by default the vulkan API has minimum error checking
  createSurface(); // creating surface that relying on GLWF. a connection between windows and vulkan image result
  pickPhysicalDevice(); // picking devices our application will be using (graphic device)
  createLogicalDevice(); // creating features our physical device want to use
  createCommandPool(); // helping command buffer allocation
}
